...
def tensor_to_pils(x):
    ...
    pils = []
    for x_ in x:
        np_x = norm(x_).permute(1, 2, 0).cpu().detach().numpy() * 255
...
def tensor_to_numpy(x):
    ...
    """
    np_images = norm(x).permute(0, 2, 3, 1).cpu().detach().numpy() * 255
...
def log_results(args, sde_trajs, results, images, y, full_samples, table_markdown, total_number):
    ...
    if args.name == 'coloring_demo':
        resized_y = resized_y.repeat(1, 3, 1, 1) / 3.
    ...
    if args.save_samples:
        ...
        measurement_list = tensor_to_pils(resized_y)
        ...
        for run in range(args.num_runs):
            for idx in range(total_number):
                ...
                measurement_path = measurement_dir / '{:05d}_run{:04d}.png'.format(idx, run)
                pil_image_list[cnt].save(str(image_path))
                measurement_list[cnt].save(str(measurement_path))
                cnt += 1
...
def sample_in_batch(sampler, model, x_start, operator, y, evaluator, verbose, record, batch_size, gt, GDAPS_scale):
    """
    ...
    for s in range(0, len(x_start), batch_size):
        ...
        cur_samples = sampler.sample(model, cur_x_start, operator, cur_y, evaluator, verbose=verbose, record=record, gt=cur_gt, GDAPS_scale=GDAPS_scale)
...
def main(args):
    ...
    full_samples = []
    full_trajs = []
    GDAPS_scale = getattr(args.task.GDAPS_config, 'scale', -0.1) if hasattr(args.task, 'GDAPS_config') else -0.1
    for r in range(args.num_runs):
        ...
        samples, trajs = sample_in_batch(sampler, model, x_start, operator, y, evaluator, verbose=True,
                                         record=args.save_traj, batch_size=args.batch_size, gt=images, GDAPS_scale=GDAPS_scale)
