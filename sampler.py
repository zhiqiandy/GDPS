...
class DiffusionSampler(nn.Module):
    ...
    def sample(self, model, x_start,measurement,operator, SDE=False, record=False, verbose=False, GDAPS_scale=1.0):
        """
        ...
        if self.solver == 'euler':
            return self._euler(model, x_start,measurement,operator, SDE, record, verbose, GDAPS_scale)
        else:
            raise NotImplementedError

    def _euler(self, model, x_start,measurement,operator, SDE=False, record=False, verbose=False, GDAPS_scale=1.0):
        """
        ...
        x = x_start
        self.ra = resample(scale=GDAPS_scale)
        for step in pbar:
...
class resample(nn.Module):
    ...
    def update(self, x, y, operator):
        x0hat = x
        norm = torch.linalg.norm(y - operator(x0hat))
        grad = torch.autograd.grad(outputs=norm, inputs=x, retain_graph=True)[0].detach()
        x = x - self.scale * grad
        return x.detach()
...
class DAPS(nn.Module):
    ...
    def sample(self, model, x_start, operator, measurement, evaluator=None, record=False, verbose=False, GDAPS_scale=1.0, **kwargs):
        """
        ...
        for step in pbar:
            ...
            x0hat = sampler.sample(model, xt,measurement,operator, SDE=False, verbose=False, GDAPS_scale=GDAPS_scale)
